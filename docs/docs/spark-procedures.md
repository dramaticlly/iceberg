...
| `output-spec-id` | current partition spec id | Desired partition spec id to be used for rewrite data files. This allow data files to be rewritten with custom partition spec |
| `remove-dangling-deletes` | false | Remove dangling position and equality deletes after rewrite. A delete files is consider dangling if it does not apply to any live data files. Enable this will generate one additional snapshot of delete type |
...
Rewrite the data files in table `db.sample` using bin-pack strategy in any partition where at least 2 files need to be rewritten and also remove dangling delete files.
```sql
